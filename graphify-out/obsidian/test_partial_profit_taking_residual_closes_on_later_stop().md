---
source_file: "tests/test_backtest_execution.py"
type: "code"
community: "Backtest Execution Tests"
location: "L356"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/Backtest_Execution_Tests
---

# test_partial_profit_taking_residual_closes_on_later_stop()

## Connections
- [[BacktestExecutionEngine]] - `calls` [INFERRED]
- [[TradeDecision]] - `calls` [INFERRED]
- [[TradingConfig]] - `calls` [INFERRED]
- [[test_backtest_execution.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/INFERRED #community/Backtest_Execution_Tests