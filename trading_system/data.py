...
class MarketDataService:
    ...
    def build_universe(self, include_news: bool = False) -> list[SymbolMarketData]:
        companies = self._build_symbol_universe()
        results: list[SymbolMarketData] = []
        failures: list[str] = []
        self.logger.info("Building universe for %s symbols", len(companies))
        ...
        if len(results) < min(100, self.config.top_universe_size // 2):
            raise DataIngestionError(
    ...
    def fetch_symbol_market_data(
        self, symbol: str, market_cap: float | None, include_news: bool = False
    ) -> SymbolMarketData:
        cached = self._read_symbol_cache(symbol)
        daily: pd.DataFrame
        premarket: PremarketSnapshot

        try:
            daily = self._fetch_daily_bars(symbol)
            premarket = self._fetch_premarket_snapshot(symbol, safe_float(daily["Close"].iloc[-1]))
        except Exception as exc:
            if cached:
                self.logger.warning("Using cached market data for %s after fetch failure: %s", symbol, exc)
                return self._symbol_market_data_from_cache(cached)
            raise

        indicators = self._compute_indicators(daily)
        close = safe_float(daily["Close"].iloc[-1])
        volume = safe_float(daily["Volume"].iloc[-1])
        high_20d = safe_float(daily["High"].tail(20).max())
        low_20d = safe_float(daily["Low"].tail(20).min())

        premarket = self._fetch_premarket_snapshot(symbol, close)
        news_headlines = self._fetch_news_headlines(symbol) if include_news else []
        price_summary, raw_metrics = self._build_price_summary(symbol, daily, premarket, indicators)
        ...
        self._write_symbol_cache(symbol_data)
        return symbol_data

    def _fetch_daily_bars(self, symbol: str) -> pd.DataFrame:
        if self.config.alpaca_api_key and self.config.alpaca_secret_key:
            try:
                daily = self._fetch_daily_bars_alpaca(symbol)
                if not daily.empty:
                    return daily
            except Exception as exc:  # noqa: BLE001
        ...
        if len(daily) < 30:
            raise DataIngestionError(f"Insufficient daily history for {symbol}")
        return daily

    def _fetch_daily_bars_alpaca(self, symbol: str) -> pd.DataFrame:
        ...
        if not bars:
            raise DataIngestionError(f"No Alpaca daily bars returned for {symbol}")
        ...
        frame = frame.dropna().tail(220)
        if len(frame) < 30:
            raise DataIngestionError(f"Insufficient Alpaca daily history for {symbol}")
        return frame

    def _fetch_premarket_snapshot(self, symbol: str, last_close: float) -> PremarketSnapshot:
        if self.config.alpaca_api_key and self.config.alpaca_secret_key:
            try:
    ...
    def _write_symbol_cache(self, symbol_data: SymbolMarketData) -> None:
        write_json(
            self.symbol_cache_dir / f"{symbol_data.symbol}.json",
